...
class VCContacter: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        var btnAddImg = UIImage(named: "添加联系人按钮图标")
        btnAddImg = btnAddImg?.withRenderingMode(.alwaysOriginal)
        let btnAdd = UIBarButtonItem(image: btnAddImg, style: .done, target: self, action: #selector(addBtnAction))
        self.navigationItem.rightBarButtonItem = btnAdd
        
        // 下一页左侧返回按钮的属性设置
        let btnBack = UIBarButtonItem(title: "通讯录", style: .plain, target: self, action: nil)
